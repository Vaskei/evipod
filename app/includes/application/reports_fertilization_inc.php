...
if (isset($_POST['fertilizationPDF'])) {
  ...
  $pdf->SetTitle('Izvještaj - gnojidba');
  $pdf->SetSubject('Izvještaj - gnojidba');
  $pdf->SetKeywords('TCPDF, PDF');

  // set default header data
  $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, html_entity_decode($currentBusiness['business_name']) . ' - Gnojidba izvještaj', PDF_HEADER_STRING, array(111, 125, 126), array(111, 125, 126));
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    <td>{$row['fertilization_name']}</td>
    <td>" . date('d. m. Y.', strtotime($row['fertilization_date'])) . "</td>
    <td>{$row['fertilization_amount']}</td>
    <td>{$row['fertilization_note']}</td>
    </tr>";
  }
  $html .= '</table>';
  ...
  $pdf->Output($currentBusiness['business_name'] . '_gnojidba_izvjestaj.pdf', 'D');
} else {
  header('Location: ../../');
}
