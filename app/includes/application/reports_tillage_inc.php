...
if (isset($_POST['tillagePDF'])) {
  ...
  $pdf->SetTitle('Izvještaj - obrada tla');
  $pdf->SetSubject('Izvještaj - obrada tla');
  $pdf->SetKeywords('TCPDF, PDF');

  // set default header data
  $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, html_entity_decode($currentBusiness['business_name']) . ' - Obrada tla izvještaj', PDF_HEADER_STRING, array(111, 125, 126), array(111, 125, 126));
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    <td>{$row['tillage_name']}</td>
    <td>" . date('d. m. Y.', strtotime($row['tillage_date'])) . "</td>
    <td>{$row['tillage_note']}</td>
    </tr>";
  }
  $html .= '</table>';
  ...
  $pdf->Output($currentBusiness['business_name'] . '_obrada_tla_izvjestaj.pdf', 'D');
} else {
  header('Location: ../../');
}
