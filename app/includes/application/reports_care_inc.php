...
if (isset($_POST['carePDF'])) {
  ...
  $pdf->SetTitle('Izvještaj - njega usjeva/nasada');
  $pdf->SetSubject('Izvještaj - njega usjeva/nasada');
  $pdf->SetKeywords('TCPDF, PDF');

  // set default header data
  $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, html_entity_decode($currentBusiness['business_name']) . ' - Njega usjeva/nasada izvještaj', PDF_HEADER_STRING, array(111, 125, 126), array(111, 125, 126));
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    <td>{$row['care_name']}</td>
    <td>{$row['care_culture']}</td>
    <td>" . date('d. m. Y.', strtotime($row['care_date'])) . "</td>
    <td>{$row['care_note']}</td>
    </tr>";
  }
  $html .= '</table>';
  ...
  $pdf->Output($currentBusiness['business_name'] . '_njega_usjeva_nasada_izvjestaj.pdf', 'D');
} else {
  header('Location: ../../');
}
