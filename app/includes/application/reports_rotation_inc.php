...
if (isset($_POST['rotationPDF'])) {
  ...
  $pdf->SetTitle('Izvještaj - Plodored');
  $pdf->SetSubject('Izvještaj - Plodored');
  $pdf->SetKeywords('TCPDF, PDF');

  // set default header data
  $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, html_entity_decode($currentBusiness['business_name']) . ' - Plodored izvještaj', PDF_HEADER_STRING, array(111, 125, 126), array(111, 125, 126));
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    <td>{$row['rotation_year']}.</td>
    <td>{$row['rotation_name']}</td>
    <td>{$row['rotation_note']}</td>
    </tr>";
  }
  $html .= '</table>';
  ...
  $pdf->Output($currentBusiness['business_name'] . '_plodored_izvjestaj.pdf', 'D');
} else {
  header('Location: ../../');
}
