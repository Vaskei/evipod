...
if (isset($_POST['protectionDelete'])) {
  // var_dump($_POST);
  // var_dump($_SESSION);
  // exit();
  $userId = $_SESSION['user_id'];
  $protectionId = $_POST['protectionDelete'];
  ...
  if ($currentBusinessId != NULL) {
    $query = $conn->prepare("DELETE protection FROM protection INNER JOIN fields ON protection.field_id = fields.field_id WHERE protection.protection_id = ? AND fields.business_id = ?");
    $query->bind_param("ii", $protectionId, $currentBusinessId);
    $query->execute();
    if ($query->affected_rows >= 1) {
      redirectWithToastSuccess("info", "Uspjeh.", "Zaštita obrisana.", "../../activities");
    } else {
