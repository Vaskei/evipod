...
if (!isset($_GET['email']) || !isset($_GET['token'])) {
  ...
  $query = $conn->prepare("SELECT * FROM users WHERE user_email=? AND tokenConfirm=? AND isEmailConfirmed=?");
  ...
  if ($query->execute()) {
    ...
    if ($result->num_rows > 0) {
      ...
      redirectWithMsg("danger", "Podaci registracije se ne poklapaju!", "./membership");
    }
  } else {
